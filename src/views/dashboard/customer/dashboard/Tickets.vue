...
const tableData = [
  { column1: 'Row 1, Col 1', column2: 'Row 1, Col 2', column3: 'Row 1, Col 3', column4: 'Row 1, Col 4', priority: 'High', status: 'open'  },
  { column1: 'Row 2, Col 1', column2: 'Row 2, Col 2', column3: 'Row 2, Col 3', column4: 'Row 2, Col 4', priority: 'High', status: 'close' },
  { column1: 'Row 3, Col 1', column2: 'Row 3, Col 2', column3: 'Row 3, Col 3', column4: 'Row 3, Col 4', priority: 'Medium', status: 'open'  },
  { column1: 'Row 3, Col 1', column2: 'Row 3, Col 2', column3: 'Row 3, Col 3', column4: 'Row 3, Col 4', priority: 'High', status: 'close' },
  { column1: 'Row 3, Col 1', column2: 'Row 3, Col 2', column3: 'Row 3, Col 3', column4: 'Row 3, Col 4', priority: 'Medium', status: 'open'  },
  { column1: 'Row 3, Col 1', column2: 'Row 3, Col 2', column3: 'Row 3, Col 3', column4: 'Row 3, Col 4', priority: 'Medium', status: 'open'  },
  { column1: 'Row 3, Col 1', column2: 'Row 3, Col 2', column3: 'Row 3, Col 3', column4: 'Row 3, Col 4', priority: 'Medium', status: 'close' },
];
...
<template>
     <!-- Header -->
    <div class="py-2">
         <Header header-title="sidebar.support_tickets"/>
    </div>
    <!-- Centered Content -->
   <div class="flex justify-between mt-2">
        <div class="lg:w-[70%] w-[95%] mx-auto lg:mx-0 overflow-auto" >
            <div class="relative">
            <img src="../../../../assets/images/customer_dashboard/background.svg" alt="">
            <div class="absolute top-0 p-4 w-full">
                <div class="flex justify-between items-center">
                    <h2 class="text-white lg:mb-4 lg:text-xl">
                        {{ t('supportTickets.ticket') }}
                    </h2>
                    <a @click="openWalletPopup"
                    href="#"
                    class="bg-white mt-1 text-black lg:text-sm text-xs rounded-full py-1 lg:ps-3 ps-2 inline-flex items-center"
                    >
                    <button class="me-1 whitespace-nowrap">{{ t('supportTickets.button') }}</button>
                    <img
                ...
                <h2 class="text-white lg:text-xl text-wrap">
                    {{ t('supportTickets.ticket_search_note') }}<br/>{{ t('supportTickets.ticket_note') }}
                </h2>
            </div>
        </div>
        <div class="bg-[#E2ECC3] mt-8 rounded-t-3xl flex items-center gap-8 rounded-b-lg py-1.5 px-4">
        <p class="font-bold">{{ t('supportTickets.all_tickets') }}</p>
                <!-- Search Input -->
        ...
        <thead>
            <tr>
            <th class="font-medium px-4 py-2 text-right border-b flex items-center gap-1"><img src="../../../../assets/icons/general/circle.svg" alt=""><span>{{ t('wallet.code') }}</span></th>
            <th class="font-medium px-4 py-2 text-right border-b">{{ t('supportTickets.ticket_title') }}</th>
            <th class="font-medium px-4 py-2 text-right border-b">{{ t('supportTickets.ticket_priority') }}</th>
            <th class="font-medium px-4 py-2 text-right border-b">{{ t('supportTickets.ticket_status') }}</th>
        ...
        <tbody>
            ...
            <td class="px-4 py-2 border-b">مشكلة في تسجيل الدخول</td>
            <td class="px-4 text-[#787474] py-2 border-b">
                {{ row.priority === 'High' ? t('supportTickets.priority_high') : t('supportTickets.priority_medium') }}
            </td>
            <td class="px-4 py-2 border-b" :class="row.status === 'open' ? 'text-[#87C101]' : 'text-[#DC3545]'">
                {{ row.status === 'open' ? t('supportTickets.status_open') : t('supportTickets.status_close') }}
            </td>
     ...
     <!-- Right Duties -->
      <div class="w-[28%] hidden lg:block overflow-y-scroll" style="scrollbar-width: none; -ms-overflow-style: none;">
         ...
         <!-- Wallet Ticket Popup -->
         <div v-if="showTicketPopup" class="overlay fixed inset-0 bg-black bg-opacity-80 z-50"></div>
        <div v-if="showTicketPopup" class="popup fixed top-1/2 left-1/2 transform space-y-4 -translate-x-1/2 -translate-y-1/2 z-50 mx-auto bg-white rounded-[20px] w-[30%]">
            ...
            <div class="pb-6 px-4">
                ...
                <div class="mt-4 flex justify-center items-center gap-4 text-sm">
                    <div class="bg-black text-white rounded-[100px]">
                        <button @click="closeWalletPopup" class="w-full px-6 py-1.5">{{ t('supportTickets.create_ticket') }}</button>
                    </div>
                    <div class="border border-black rounded-[100px]">
                        <button @click="closeWalletPopup" class="w-full px-6 py-1.5">{{ t('delete_popup.cancel') }}</button>
                    </div>
                </div>
            </div>
        </div>
